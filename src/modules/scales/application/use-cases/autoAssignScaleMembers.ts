...
export async function autoAssignScaleMembers(
  ...
  const [church, service, roles, memberChurches, memberMinistries] =
    await Promise.all([
  ...
  const serviceById = new Map(
    churchServices.map((churchService) => [churchService.id, churchService]),
  );
  ...
  for (const scaleMember of scaleMembers) {
    ...
    const assignedMembers =
      memberIdsByServiceId.get(scale.serviceId) ?? new Set<string>();
  ...
  const previousServices = churchServices
    ...
    .filter(
      (churchService) =>
        toServiceSortKey(churchService.date, churchService.time) <
        currentServiceKey,
    )
    .sort((a, b) =>
      toServiceSortKey(b.date, b.time).localeCompare(
        toServiceSortKey(a.date, a.time),
      ),
    );
  ...
  for (const member of activeCandidates) {
    ...
    for (const previousService of previousServices) {
      ...
      const serviceKey = toServiceSortKey(
        previousService.date,
        previousService.time,
      );
      if (!lastAssignedAt) {
  ...
  for (const role of orderedRoles) {
    for (let index = 0; index < role.requiredCount; index += 1) {
      const eligibleCandidates = activeCandidates
        ...
        .filter(
          (member) => (memberStreak.get(member.id) ?? 0) < maxConsecutiveScales,
        )
        .sort((a, b) => {
          const streakDiff =
            (memberStreak.get(a.id) ?? 0) - (memberStreak.get(b.id) ?? 0);
          if (streakDiff !== 0) {
            return streakDiff;
          }
