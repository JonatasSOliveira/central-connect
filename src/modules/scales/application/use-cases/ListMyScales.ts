...
export class ListMyScales extends BaseUseCase<
  ListMyScalesInput,
  ListMyScalesOutput
> {
  constructor(
  ...
  async execute(input: ListMyScalesInput): Promise<Result<ListMyScalesOutput>> {
    try {
      ...
      const scaleIds = Array.from(
        new Set(memberScales.map((item) => item.scaleId)),
      );
      const scales = (
        await Promise.all(
      ...
      const roleMap = new Map(
        ministryRoles.flat().map((role) => [role.id, role]),
      );

      const scaleMap = new Map(
        publishedScales.map((scale) => [scale.id, scale]),
      );
