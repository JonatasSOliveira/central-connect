import type { MemberProfileFilters } from "../dtos/member-profile/MemberProfileDTO";
...
export function matchesMemberProfileFilters(
  ...
  const currentMinistryIds = record.currentMinistries.map(
    (item) => item.ministryId,
  );
  const desiredMinistryIds = record.desiredMinistries.map(
    (item) => item.ministryId,
  );
  const availabilitySlots = record.serviceAvailabilities.map(
    (item) => item.slot,
  );
  const drivingSkill = record.practicalSkills.find(
    (item) => item.hasDriverLicense !== null || item.hasOwnVehicle !== null,
  );
  ...
  return (
    ...
    matchesAny(
      filters.discipleshipStatus,
      record.spiritualJourney?.discipleshipStatus,
    ) &&
    matchesAny(
      filters.smallGroupStatus,
      record.spiritualJourney?.smallGroupStatus,
    ) &&
    matchesAny(
      filters.churchAttendanceTime,
      record.spiritualJourney?.churchAttendanceTime,
    ) &&
    matchesAny(
      filters.officialMemberStatus,
      record.spiritualJourney?.officialMemberStatus,
    ) &&
